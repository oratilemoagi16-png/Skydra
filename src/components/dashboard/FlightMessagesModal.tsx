...
export function FlightMessagesModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center p-4 overflow-y-auto mobile-safe-container">
      ...
      <div
        ref={modalRef}
        className="flight-messages-modal relative z-10 w-full max-w-lg bg-elevated border border-line rounded-2xl shadow-2xl flex flex-col max-h-[80vh] modal-mobile-max my-auto"
        role="dialog"
        aria-modal="true"
        aria-label={t('map.messages')}
      >
        {/* Header */}
        <div className="flex items-center px-5 py-4 border-b border-line flex-shrink-0 gap-3">
          ...
          <div className="flex items-center gap-2">
            {cautionCount > 0 && (
              <span className="inline-flex items-center gap-1.5 text-[13px] font-medium px-2.5 py-1 rounded-full bg-danger/15 text-danger border border-danger/30">
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-1.5 rounded-lg text-muted hover:text-ink hover:bg-line/60 transition-colors"
        ...
        {/* Column labels */}
        <div className="grid grid-cols-[56px_48px_1fr] sm:grid-cols-[72px_64px_1fr] gap-x-2 sm:gap-x-3 px-3 sm:px-5 py-2 border-b border-line/50 flex-shrink-0">
          <span className="text-[10px] font-medium uppercase tracking-wide text-muted msg-col-label">
        ...
        <div className="overflow-y-auto flex-1 min-h-0 divide-y divide-line/60 px-0">
          {sorted.length === 0 ? (
            ...
            sorted.map((msg, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={`grid grid-cols-[56px_48px_1fr] sm:grid-cols-[72px_64px_1fr] gap-x-2 sm:gap-x-3 items-baseline px-3 sm:px-5 py-3 transition-colors ${isCaution ? 'hover:bg-danger/10' : isWarn ? 'hover:bg-warning/10' : 'hover:bg-accent/10'
                    }`}
                >
                  {/* Clock time */}
                  <span className="text-[11px] font-medium text-ink tabular-nums leading-tight msg-time">
                    {formatClockTime(flightStartTime, msg.timestampMs, hour12)}
                  </span>

                  {/* Flight time */}
                  <span className="text-[11px] tabular-nums text-muted leading-tight msg-flight-time">
                  ...
                  <div className="flex items-start gap-2 min-w-0">
                    ...
                    <span
                      className={`text-sm leading-snug break-words min-w-0 msg-text ${isCaution ? 'text-danger msg-text-caution' : isWarn ? 'text-warning msg-text-warn' : 'text-accent msg-text-tip'
                        }`}
                    >
                      {msg.message}
                    </span>
