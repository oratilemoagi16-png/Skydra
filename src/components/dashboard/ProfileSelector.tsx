  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-1 text-xs py-1.5 px-2 rounded-lg border border-line text-muted hover:text-ink hover:border-line-strong transition-colors max-w-[100px]"
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 w-56 max-w-[calc(100vw-1rem)] bg-surface border border-line rounded-lg shadow-xl z-[100] overflow-hidden">
          ...
          {passwordPrompt && !confirmingDelete && (
            <div className="p-3">
              ...
              <PasswordInput
                ...
                className="w-full text-xs px-2 py-1.5 bg-canvas border border-line-strong rounded text-ink placeholder:text-faint focus:outline-none focus:border-accent"
              />
              {error && <p className="text-[10px] text-danger mt-1 px-0.5">{error}</p>}
              <div className="flex gap-1 mt-1.5">
                <button
                  onClick={handleSwitchWithPassword}
                  disabled={busy}
                  className="flex-1 text-[10px] py-1 rounded bg-accent/15 border border-accent text-ink hover:bg-accent/25 transition-colors disabled:opacity-50 flex items-center justify-center gap-1"
                >
                  {busy ? <Spinner /> : null}
                  {t('profile.unlock')}
                </button>
                <button
                  ...
                  disabled={busy}
                  className="flex-1 text-[10px] py-1 rounded border border-line-strong text-muted hover:text-ink transition-colors disabled:opacity-50"
                >
                  {t('profile.cancel')}
                </button>
              </div>
          ...
          {confirmingDelete && (
            <div className="p-3">
              ...
              {error && <p className="text-[10px] text-danger mt-1 mb-1 px-0.5">{error}</p>}
              <div className="flex gap-1.5">
                <button
                  onClick={confirmDelete}
                  disabled={busy}
                  className="flex-1 text-[10px] py-1.5 rounded bg-danger/15 border border-danger text-danger hover:bg-danger/30 hover:text-ink transition-colors font-medium disabled:opacity-50 flex items-center justify-center gap-1"
                >
                  {busy ? <Spinner /> : null}
                  {t('profile.delete')}
                </button>
                <button
                  ...
                  disabled={busy}
                  className="flex-1 text-[10px] py-1.5 rounded border border-line-strong text-muted hover:text-ink transition-colors disabled:opacity-50"
                >
                  {t('profile.cancel')}
                </button>
              </div>
              ...
              <div
                ...
                className={`flex items-center justify-between px-3 py-2 text-xs cursor-pointer transition-colors ${
                  name === activeProfile
                    ...
                    : 'text-ink hover:bg-line/50 hover:text-ink'
                }`}
              >
                <span className="truncate flex-1 flex items-center gap-1">
                  {name === 'default' ? t('profile.default') : name}
                  {profilePasswords[name] && (
                    <svg className="w-2.5 h-2.5 text-warning flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
          ...
          {showNewInput ? (
            <div className="p-2">
              ...
              {error && (
                <p className="text-[10px] text-danger mt-1 px-0.5">{error}</p>
              )}
              <div className="flex gap-1 mt-1.5">
                <button
            ...
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2 text-xs text-muted hover:text-ink hover:bg-line/50 transition-colors"
            >
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="border-t border-line">
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2 text-xs text-muted hover:text-danger hover:bg-line/50 transition-colors"
            >
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
